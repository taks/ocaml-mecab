(* OASIS_START *)
(* DO NOT EDIT (digest: 5c48174419f30d940f9b4372ee8000c9) *)
...
let package_default =
  {
     MyOCamlbuildBase.lib_ocaml = [("lib/Mecab", ["lib"])];
     lib_c = [("Mecab", "lib", [])];
     flags =
       [
          (["oasis_library_mecab_cclib"; "link"],
            [
               (OASISExpr.EBool true,
                 S [A "-cclib"; A "-lmecab"; A "-cclib"; A "-lstdc++"])
            ]);
          (["oasis_library_mecab_cclib"; "ocamlmklib"; "c"],
            [(OASISExpr.EBool true, S [A "-lmecab"; A "-lstdc++"])])
       ];
     }
  ;;
